...
for key, value in any_KRs.items():
    ...
    for subkey in value.keys():
        if 'plot' in subkey:
            sub_plot_list.append(any_KRs[key][subkey].items())
            print sub_plot_list
...
plt6 = Plotter(reader, 'sensors.imu.6', subkeys=[(('angularVelocity.x', 'x'), ('angularVelocity.y', 'y')), (('angularVelocity.x', 'x'), ('angularVelocity.y', 'y'))], points_per_plot=10)
